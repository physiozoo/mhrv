...
% === Aviv
% Extract only the filename itself, and treat user input as path. This way
% the output files can be written to any directory, not just pwd.
fname_path = fname;
...
%Write *.dat file
fid = fopen([fname_path '.dat'],'wb',machine_format);
if(~fid)
